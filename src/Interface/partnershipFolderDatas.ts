export const optionTransformer = function (value: string) {
  return {
    text: value,
    value: value,
  };
};
export const optionTransformerSkipFirst = function (
  value: string,
  index: number
) {
...
export const partnerFolderRelations = [
  "paymentModes",
  "deliveryModes",
  "sites",
  "campaignOptions",
...
export const siteRelations = ["siteTrees", "banners", "languages"];
...
export const restoEngineOptions = ["Libro", "Reservit", "Guestonline", ""].map(
  optionTransformer
);

export const noSiteCauseList = ["Temps", "Budget"].map(optionTransformer);

export const noSiteObjectiveList = ["Visibilité", "Crédibilité"].map(
  optionTransformer
);
...
export const newBoutiquePaymentMeth = ["En ligne", "En magasin"].map(
  optionTransformer
);
...
export const transporterOptions = ["Au poids", "Au panier"].map(
  optionTransformer
);

export const excelProductList = {
  title:
    "Remplissez le fichier Excel transmis et listez les produits (20 max) que vous souhaitez que l'on mette en place pour la livraison de votre boutique. Pour chacun de vos produits, il nous faudra :",
  ...
  fileLocalShop:
    "assets/files/parntership-file/Matrice_DossierPartenaire_V4.xlsx",
};

export const commonPortals = ["LeBonCoin", "ParuVendu", "Autre"].map(
  optionTransformer
);

export const autoPortals = ["La centrale"].map(optionTransformer);

export const immoPortals = ["Se Loger"].map(optionTransformer);
...
export const hasLogoChoice = ["Oui", "Non"].map(optionTransformer);

export const todoWithLogoOptions = ["Oui", "Non"].map(optionTransformer);
...
export const memberType = ["Manager", "Équipier"].map(optionTransformer);

export const visioServiceExist = ["Sur place", "En visio"].map(
  optionTransformer
);

export const availableVats = ["0", "5,5", "10", "20"].map(optionTransformer);
