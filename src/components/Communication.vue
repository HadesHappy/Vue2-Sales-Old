...
var baseUrl = import.meta.env.SALESFORCE_ROOT_API;
export default {
    name: "Profil",
    ...
    methods: {
        ...
        **/
        ...
        retrieve_business_info: function () {
            var url = baseUrl +  import.meta.env.SALESFORCE_QUERY_BASE_SOBJECTS + "Account/" + SALESFORCE_ACCOUNT_ID;
            const authStr = 'Bearer '.concat(SALESFORCE_USER_TOKEN);
            axios.get(url, { headers: { Authorization: authStr } }).then(response => {
                businessName = response.data.Raison_sociale__c;
            }).catch(error => {
        ...
        retrieve_shiping_info: function () {
            var url = baseUrl +  import.meta.env.SALESFORCE_QUERY_BASE_SOBJECTS + "Contact/" + SALESFORCE_CONTACT_ID;
            const authStr = 'Bearer '.concat(SALESFORCE_USER_TOKEN);
            axios.get(url, { headers: { Authorization: authStr } }).then(response => {
                firstName = response.data.FirstName;
                lastName = response.data.LastName;
                genre =  response.data.genre;
                shippingAddress = response.data.MailingStreet;
                shippingCity = response.data.MailingCity;
                shippingPostalCode = response.data.MailingPostalCode;
                shippingPhone = response.data.Phone;
                shippingMail = response.data.email;
                shippingMobilePhone = response.data.MobilePhone;
            }).catch(error => {
                console.warn('error ' + error);
                window.location.href = '/#/authentification';
            });
        ...
        update_infos: function () {
            ...
            var mailingStreet = shippingAddress;
            var mailingPostalCode = shippingPostalCode;
            var phone = shippingPhone;
            var mobilePhone = shippingMobilePhone;
            var email = shippingMail;
            var url = baseUrl + import.meta.env.SALESFORCE_QUERY_BASE_SOBJECTS + "Contact/" + SALESFORCE_CONTACT_ID;
            const authStr = 'Bearer '.concat(SALESFORCE_USER_TOKEN);
            const body = {
                "FirstName": firstName,
