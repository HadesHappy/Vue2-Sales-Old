<template>
	<div id="authentification">
		<div
			v-if="status.loggingIn || store.state.globalStore.status.isPasswordResetting"
		...
		<b-container fluid class="bv-example-row">
			<b-row>
				...
				<b-col class="img-section" offset-lg="1" lg="6">
					<img
						class="image-left-auth"
						src="@/assets/visuel-authentification.png"
					/>
				</b-col>
				<b-col
					@keyup.enter="authenticate"
					v-if="!$store.state.globalStore.status.passwordResetForm"
					lg="4"
					offset-lg="0"
					offset-sm="3"
					class="labels-input"
				>
					<b-row>
						<b-col offset-lg="1" lg="8" md="6" sm="6">
							<b-row>
								<b-col lg="7" md="12" sm="12" class="id-section"
								>Identifiant
								</b-col
								>
						...
						<b-col offset-lg="1" lg="8" md="6" sm="6">
							<b-row>
								...
								<p class="frgt-pwd" @click="togglePasswordResetForm(true)">
									Mot de passe oublié ?
								</p>
							</b-row>
						</b-col>
						<b-col offset-lg="1" lg="8" md="6" sm="6">
							<b-row>
								...
								<b-col lg="4" md="12" sm="12" class="id-section">
									<button
										@click="authenticate"
										class="uppercase auth"
										type="submit"
										v-bind:disabled="status.loggingIn"
									>
										Se connecter
									</button>
								</b-col>
				...
				>
					<b-row>
						<b-col offset-lg="2" lg="12" md="6" sm="6">
							<b-row>
								<b-col lg="8" md="12" sm="12" class="id-section"
								...
								<b-col lg="6" md="12" sm="12" class="id-section">
									<input
										...
										v-model="emailReset"
										class="forgot-pswd-input"
										required
									/>
								</b-col>
								<b-col lg="8" md="6" sm="6">
									<p class="frgt-pwd" @click="togglePasswordResetForm(false)">
									...
									<button
										...
										type="submit"
										v-bind:disabled="
                      $store.state.globalStore.status.isPasswordResetting
                    "
									>
										Réinitialiser
									</button>
...
onMounted(() => {
	resetPartnerOffers();
	if (null === route.query["mot-de-passe-oublie"]) {
		togglePasswordResetForm(false);
	}
});

const login = (data: any) => store.dispatch("account/login", data);
const logout = () => store.dispatch("account/logout");
const resetPartnerOffers = () =>
	store.dispatch("globalStore/resetPartnerOffers");
const sendResetPasswordMail = (value: any) =>
	store.dispatch("globalStore/sendResetPasswordMail", value);
const resetState = () => store.dispatch("globalStore/resetState");

const togglePasswordResetForm = (value: boolean) =>
	store.commit("globalStore/togglePasswordResetForm", value);

const authenticate = () => {
	const trimedUsernmae = username.value.trim();
	const trimedPassword = password.value.trim();

	login({
		username: trimedUsernmae,
