...
function sendResetPasswordMail(data) {
	return requestHelpers.axiosInstance.post(
		...
		"/localfr-api/send-reset-pwd-link",
		data,
		{
			headers: {
...
function sendNewBillingInfoMail(data) {
	return requestHelpers.securedAxiosInstance.post(
		import.meta.env.VITE_LOCALETMOI_BASE_URL + "/api/mandrill/billing-infos",
		data,
		{
			headers: {
				"Content-type": "application/json",
...
function sendDropboxNotification(customerCode, source, folders) {
	return requestHelpers.securedAxiosInstance.post(
		import.meta.env.VITE_LOCALETMOI_BASE_URL +
		"/api/mandrill/notification/duplicate-folders",
		{
			customerCode,
			source,
			folders,
...
function notifyUploadedFiles(customerCode, source, files, folderName) {
	return requestHelpers.securedAxiosInstance.post(
		import.meta.env.VITE_LOCALETMOI_BASE_URL +
		"/api/mandrill/notification/upload-files",
		{
			customerCode,
			source,
			files,
