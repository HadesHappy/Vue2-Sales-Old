import {getParsedLocalStorageItem, requestHelpers} from "@/helpers";
...
function getAutologin(customerCode, userEmail, websiteLink) {
	return requestHelpers.securedAxiosInstance
		.get(`${import.meta.env.VITE_LOCALETMOI_BASE_URL}/teds/authorization`, {
			params: {
...
function getRefreshToken(customerCode, url) {
	const userBoutique = getParsedLocalStorageItem("userBoutique");
	return requestHelpers.axiosInstance
		.post(
			`${import.meta.env.VITE_LOCALETMOI_BASE_URL}/teds/refresh-user-token`,
			{
				userBoutique,
				customerCode,
				url,
