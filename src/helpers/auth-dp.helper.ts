...
function hasRole(identity, role) {
	...
	//console.log('has role checking',identity.roles, role, identity.roles.indexOf(role));
	return -1 !== identity.roles.indexOf(role);
}

function isUser(identity) {
	//console.log(identity.roles);
	const cond = identity.roles ? hasRole(identity, "ROLE_CLIENT") || hasRole(identity, "ROLE_ADMIN") : false;
	return cond;
}
...
function isSuperAdmin(identity) {
	return (
		hasRole(identity, "ROLE_SUPER_ADMIN") ||
		hasRole(identity, "ROLE_MANAGER") ||
		hasRole(identity, "ROLE_DEV_ADMIN")
	);
}

function canEditPartnerFolder(opportunityDate, identity) {
	const dpValidatedDate = opportunityDate
		? dateHelpers.strDateToObject(opportunityDate)
		: null;
	const dpValidatedDateFormatted = dpValidatedDate
		? dpValidatedDate.toLocaleString("fr-FR").substring(0, 10)
		: null;

	console.log('[HELPER] [canEditPartnerFolder fn] identitiy ', identity);
	console.log('salesman', isSalesman(identity));
	console.log('provider',isProvider(identity));
	console.log('user', isUser(identity));
	console.log('isSUepradmin', isSuperAdmin(identity));
	console.log('dpValidatedDate',dpValidatedDate);
	console.log('[HELPER] [canEditPartnerFolder fn]  canedit ',	!dpValidatedDate ? false : dpValidatedDate < new Date());
...
function hasRoleToEditPartnerFolder(user) {
	console.log('HELPER: roles', user)
	return (
		allowedRolesToEditPartnerFolder.filter(
			(role) => -1 !== user.roles.indexOf(role)
		).length > 0
	);
}
...
function getUserStatus(user) {
	...
	return allowedRolesToTransmitPartnerFolder.find(
		(item) => -1 !== item.roles.indexOf(user.Fonction_hi_rarchique__c)
	);
}
...
function canTransmitPf() {
	...
	const allowedRoles = allowedRolesToTransmitPartnerFolder.reduce(
		(accumulator, allowedRole) => {
	...
	switch (currentUserStep) {
		case 1:
			canTransmit =
				(partnerHistoryContext.currentStep || null) < 2 &&
				!partnerHistoryContext.next &&
				!partnerHistoryContext.nextStep;
			break;
		case 2:
			canTransmit =
				((partnerHistoryContext.current &&
						(partnerHistoryContext.currentStep || null) === currentUserStep) ||
					(partnerHistoryContext.previous &&
						(partnerHistoryContext.previousStep || null) + 1 ===
						currentUserStep)) &&
				!partnerHistoryContext.next;
			break;
		case 3:
...
function canByPassRolesToUpdatePf() {
	return (
		store.state.account &&
		store.state.account.salesforceUser &&
		byPassRulesToEditDP.includes(
			store.state.account.salesforceUser.Fonction_hi_rarchique__c
		)
	);
}
