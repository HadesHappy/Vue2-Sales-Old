...
const allowedExtensions = import.meta.env.VITE_ALLOWED_UPLOAD_EXTENSIONS;

/**
 * @returns {string[]}
 */
export const getAllowedExtensions = () => {
	return allowedExtensions.split(",").map((ext: string) => `.${ext}`);
};
 ...
 * @returns {boolean}
 */
export const isFileAuthorized = (file) => {
	const fileExtension = (file.name || file.originalname)
		.split(".")
		.pop()
		.toLowerCase();
	...
	switch (true) {
		case Array.isArray(availableMimeType):
			return !!availableMimeType.find(
				(item: string) => -1 !== mimeType.indexOf(item)
			);
