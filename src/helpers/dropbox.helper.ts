...
function getFolderPath() {
	if (
		store.state.globalStore.salesforce.account.Name &&
		store.state.account.currentPartner.customerCode
	) {
		return Promise.resolve(
			`${store.state.account.currentPartner.customerCode} ${store.state.globalStore.salesforce.account.Name}`
		);
	}
...
function getMultipleFoldersList() {
	return store.state.globalStore.status.mostRecentFolder.multipleFolders
		? store.state.globalStore.status.mostRecentFolder.multipleFolders
		: [];
}

function getMultipleFoldersMessage() {
	return store.state.globalStore.status.mostRecentFolder.message
		? store.state.globalStore.status.mostRecentFolder.message
		: "";
}
...
function retrieveMostRecentBasePath() {
	...
	return !folderPath || "" === folderPath
		? null
		: this.getBasePath(folderPath.path);
}
