...
function getTicketTypes() {
  return {
    "01 CREA SITE": {
      ...
      "Corrections CREA/minimes (-30 min)":
        "Transmettre des modifications simples",
      "Corrections CREA/importantes (1h30 )":
        "Transmettre des modifications importantes",
      "Corrections CREA refonte":
        "Revoir entièrement la proposition de site transmise",
    },
    "02 MAJ SITE": {
      "Corrections MAJ/minimes (-30 min)":
        "Transmettre des modifications simples",
      "Corrections MAJ/importantes (1h30 )":
        "Transmettre des modifications importantes",
    ...
    "03 AUDIENCE": {
      ...
      "Fiche GMB / Facebook / Visibilité":
        "Une question sur mon compte visibilité",
    },
    "04 TECHNIQUE": {
    ...
    "05 DIVERS": {
      ...
      "Assistance / Tutos sur nos CMS":
        "Besoin d'une assistance pour mettre à jour mon site",
...
function getAvailableTicketTypes(isWebsiteOnline) {
  ...
  Object.keys(ticketTypes).forEach((currentCategory) => {
    if (
      ("01 CREA SITE" === currentCategory && isWebsiteOnline) ||
...
function findTicketCategory(ticketType, isWebsiteOnline) {
  ...
  return Object.keys(ticketTypes).find((currentCategory) => {
    ...
    return (
      -1 !== Object.values(ticketTypes[currentCategory]).indexOf(ticketType)
    );
  });
}
...
function findTicketType(ticketType, isWebsiteOnline) {
  ...
  Object.keys(ticketTypes).some((currentCategory) => {
    ...
    const index = Object.values(ticketTypes[currentCategory]).indexOf(
      ticketType
    );
    if (-1 !== index) {
